 ...
 */
public class InspectionUtility implements IMultiTouchEventListener,
		CircularLayoutListener {
	...
	public void cursorPressed(MultiTouchCursorEvent event) {
		...
		int y = SynergyNetDesktop.getInstance().tableToScreenY(
				event.getPosition().y);
		if (isCircularCentrePicked(event.getCursorID(), new Vector2f(x, y))) {
			return;
		}
		List<ContentItem> items = getPickedItems(event.getCursorID(),
				new Vector2f(x, y));
		if (items.size() > 1) {
			CircularLayout cl = new CircularLayout(contentSystem, items,
					new Vector3f(x, y, 0), 300);
		...
		try {
			...
			for (PickResultData pr : pickResults) {
				for (TablePortal portal : WorkspaceManager.getInstance().inspectedPortals) {
					for (ContentItem item : portal.getDisplayPanel()
							.getOnlineItems()) {
						if (item.getName().equals(
								pr.getPickedSpatial().getName())) {
		...
		try {
			...
			for (PickResultData pr : pickResults) {
				for (CircularLayout layout : layouts) {
					if (layout.centrePoint.isVisible()
							&& layout.centrePoint.getName().equals(
									pr.getPickedSpatial().getName())) {
						return true;
					}
				}
	...
	/*
	 * (non-Javadoc)
	 * @see apps.remotecontrol.tableportal.inspectionutility.CircularLayout.
	 * CircularLayoutListener
	 * #layoutCollapsed(apps.remotecontrol.tableportal.inspectionutility
	 * .CircularLayout)
	 */
	@Override
