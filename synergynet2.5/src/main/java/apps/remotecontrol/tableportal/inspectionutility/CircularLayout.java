...
public class CircularLayout {
	...
	/**
	 * The listener interface for receiving circularLayout events. The class
	 * that is interested in processing a circularLayout event implements this
	 * interface, and the object created with that class is registered with a
	 * component using the component's
	 * <code>addCircularLayoutListener<code> method. When
	 ...
	 *            the distance
	 */
	public CircularLayout(ContentSystem contentSystem, List<ContentItem> items,
			Vector3f centre, float distance) {
		lines = new ArrayList<Line>();
		originalLocsMap = new HashMap<ContentItem, Location>();
		centrePoint = (RoundFrame) contentSystem
				.createContentItem(RoundFrame.class);
		centrePoint.setRadius(20);
		centrePoint.setBackgroundColour(Color.white);
		sourcePortal = WorkspaceManager.getInstance().getSourceTablePortal(
				items.get(0));
		sourcePortal.getDisplayPanel().getWindow().addSubItem(centrePoint);
		Spatial bf = ((Spatial) WorkspaceManager.getInstance()
				.getSourceTablePortal(items.get(0)).getDisplayPanel()
				.getWindow().getBackgroundFrame().getImplementationObject());
		...
		for (ContentItem item : items) {
			originalLocsMap.put(item, new Location(item.getLocalLocation()
					.getX(), item.getLocalLocation().getY(), 0));
			positionItem(item, currentAngle);

			Line line = new Line(UUID.randomUUID().toString(), new Vector3f[] {
					new Vector3f(store.x, store.y, 0),
					new Vector3f(item.getLocalLocation().x,
							item.getLocalLocation().y, 0) }, null,
					new ColorRGBA[] { ColorRGBA.white, ColorRGBA.white }, null);
			line.setLineWidth(1f);
			line.setSolidColor(new ColorRGBA(sourcePortal.getWindow()
					.getBackgroundColour().getRed(), sourcePortal.getWindow()
					.getBackgroundColour().getGreen(), sourcePortal.getWindow()
					.getBackgroundColour().getBlue(), sourcePortal.getWindow()
					.getBackgroundColour().getAlpha()));
			...
			sourcePortal.getDisplayPanel().getWindow().getNode()
					.attachChild(line);
	...
	private void positionItem(ContentItem item, float angle) {
		...
		do {
			...
			item.setLocalLocation(oldX + (distance * FastMath.cos(angle)), oldY
					+ (distance * FastMath.sin(angle)));
			distance += 1;
