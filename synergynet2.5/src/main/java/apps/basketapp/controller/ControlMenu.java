...
public class ControlMenu {
	 ...
	 * <code>addBasketControlMenuListener<code> method. When
	...
	private ListContainer LoadControlMenu() {
		...
		sendDataButton.addButtonListener(new SimpleButtonAdapter() {
			public void buttonReleased(SimpleButton b, long id, float x,
				...
				for (BasketControlMenuListener l : listeners) {
					l.sendDesktopData();
				}
			}
		});
		...
		final SimpleButton swapBasketsButton = (SimpleButton) contentSystem
				.createContentItem(SimpleButton.class);
		...
		final SimpleButton captureTablesButton = (SimpleButton) contentSystem
				.createContentItem(SimpleButton.class);
		...
		if (contentSubMenu != null) {
			controlMenu.addSubMenu(contentSubMenu.getSubAppMenu(),
					"Choose Content");
		}
