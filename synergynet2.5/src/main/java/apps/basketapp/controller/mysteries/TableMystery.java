...
public class TableMystery {
	...
	/** The initial scale. */
	public static float MIN_SCALE = 0.25f, MAX_SCALE = 1.5f,
			INITIAL_SCALE = 0.4f;
	...
	public List<ContentItem> getMystery(ContentSystem contentSystem,
		...
		String folder = BasketAppResources.class.getResource("tables")
				.getFile() + File.separatorChar;
		...
		if (directory.isDirectory()) {
			...
			for (File file : files) {
				try {
					LightImageLabel item = (LightImageLabel) contentSystem
