...
public class BasketManager implements IMultiTouchEventListener {
	...
	public BasketManager(DefaultSynergyNetApp app) {
		pickSystem = MultiTouchInputFilterManager.getInstance()
				.getPickingSystem();
	...
	public void cursorReleased(MultiTouchCursorEvent event) {
		...
		for (ContentItem item : pickedItems) {
			if (targetBasket != null) {
				if (!targetBasket.getWindow().getAllItemsIncludeSystemItems()
						.contains(item)) {
					// put item in basket
					targetBasket.addItem(item);
					Spatial windowSpatial = (Spatial) targetBasket.getWindow()
							.getBackgroundFrame().getImplementationObject();
	...
	private Basket getPickedBasket(List<Spatial> spatials) {
		for (Basket basket : baskets) {
			for (Spatial spatial : spatials) {
				if (basket.getWindow().getName().equals(spatial.getName())
						|| basket.getWindow().getBackgroundFrame().getName()
	...
	private List<ContentItem> getPickedItems(List<Spatial> spatials) {
		...
		for (Spatial spatial : spatials) {
			if (!isBasket(spatial.getName())) {
				ContentItem item = contentSystem.getContentItem(spatial
						.getName());
				items.add(item);
			}
		}
	...
	private boolean isBasket(String objectName) {
		for (Basket basket : baskets) {
			if (basket.getWindow().getName().equalsIgnoreCase(objectName)
					|| basket.getWindow().getBackgroundFrame().getName()
	...
	public void swapBaskets() {
		if (RapidNetworkManager.getTableCommsClientService() != null) {
			for (Basket basket : baskets) {
				if (!basket.getTableId().equals(
						TableIdentity.getTableIdentity())) {
					try {
						RapidNetworkManager.postItems(basket.getItems(),
								basket.getTableId());
						Thread.sleep(1000);
						basket.clear();
					} catch (IOException e) {
