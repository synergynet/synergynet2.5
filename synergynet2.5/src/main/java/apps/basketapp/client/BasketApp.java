...
public class BasketApp extends DefaultSynergyNetApp implements BasketListener {
	...
	public void addContent() {
		...
		/*
		 * if(loadState){ StateLoader loader = new StateLoader(contentSystem);
		 * List<ContentItem> items = loader.loadItems(); if(items != null)
	 ...
	 * (non-Javadoc)
	 * @see
	 * apps.basketapp.client.Basket.BasketListener#itemAdded(synergynetframework
	 * .appsystem.contentsystem.items.ContentItem)
	 */
	@Override
	 ...
	 * (non-Javadoc)
	 * @see
	 * apps.basketapp.client.Basket.BasketListener#itemDetached(synergynetframework
	 * .appsystem.contentsystem.items.ContentItem)
	 */
	@Override
	public void itemDetached(ContentItem item) {
		if (item.getNote().equalsIgnoreCase("text")) {
			...
			((OrthoContentItem) item).setScaleLimit(TextMystery.MIN_SCALE,
					TextMystery.MAX_SCALE);
		}
	}
					...
					public void itemsReceived(List<ContentItem> items,
						...
						for (ContentItem item : items) {
							...
							if (item.getNote().equalsIgnoreCase(
									ContentSubMenu.CONTENT_MAP)) {
								MapInitializer mapInit = new MapInitializer(
										(LightImageLabel) item);
								onlineItems.addAll(mapInit
										.getMystery(contentSystem));
							} else if (item.getNote().equalsIgnoreCase(
									ContentSubMenu.CONTENT_TEXT)) {
								((OrthoContentItem) item).setScaleLimit(
										TextMystery.MIN_SCALE,
										TextMystery.MAX_SCALE);
							} else if (item.getNote().equalsIgnoreCase(
									ContentSubMenu.TABLE)) {
								((OrthoContentItem) item).setScaleLimit(
										TableMystery.MIN_SCALE,
										TableMystery.MAX_SCALE);
							}
						}
						onlineItems.addAll(items);
						if (!tableId.toString().equalsIgnoreCase("teacher")) {
							Basket basket = manager.getBasket(TableIdentity
									.getTableIdentity());
							if (basket != null) {
				...
				if (obj instanceof TableAnounceMessage) {
					...
					for (ContentItem item : onlineItems) {
						for (Basket basket : manager.getBaskets()) {
							if (basket.containsItem(item)) {
								basket.detachItem(item);
							}
						}
