...
public class FileTetrisLoader implements TetrisLoader {
	 ...
	 * (non-Javadoc)
	 * @see
	 * apps.threedpuzzle.TetrisLoader.TetrisLoader#loadTetris(java.util.List,
	 * com.jme.scene.Node)
	 */
	@Override
	public void loadTetris(List<Spatial> tetrisList, Node worldNode) {
		...
		try {
			...
			while (strLine != null) {
				...
				tetris.setLocalTranslation(new Vector3f(Float
						.parseFloat(attrs[1]), Float.parseFloat(attrs[2]), 70));

				Quaternion tq = new Quaternion();
				tq.fromAngleAxis(
						Float.parseFloat(attrs[3]),
						new Vector3f(Float.parseFloat(attrs[4]), Float
								.parseFloat(attrs[5]), Float
								.parseFloat(attrs[6])));
