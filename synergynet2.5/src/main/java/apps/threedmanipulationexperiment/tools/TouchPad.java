...
public class TouchPad {
	...
	public TouchPad(String name, ContentSystem contentSystem, Node worldNode,
		...
		/*
		 * line = new Line(name+"line"); Vector3f cursorWorldStart =
		 * DisplaySystem.getDisplaySystem().getWorldCoordinates(new
		 * Vector2f(initMonitorPosition.x, initMonitorPosition.y), 0.9f);
		 * if(cursorWorldStart != null && manipulatabledOjbect != null){
		 * FloatBuffer vectorBuff = BufferUtils.createVector3Buffer(2);
		 * FloatBuffer colorBuff = BufferUtils.createFloatBuffer(new
		 * ColorRGBA[]{ColorRGBA.gray, ColorRGBA.white});
		 * BufferUtils.setInBuffer(cursorWorldStart, vectorBuff, 0);
		 * BufferUtils.setInBuffer(manipulatabledOjbect.getLocalTranslation(),
		 * vectorBuff, 1); line.setLineWidth(1f); line.reconstruct(vectorBuff,
		 * null, colorBuff, null); line.updateRenderState();
		 * line.updateGeometricState(0f, false); }
		 * this.worldNode.attachChild(line); worldNode.updateGeometricState(0f,
		 * false);
		 */
	...
	public void updateLine() {
		...
		/*
		 * Vector3f cursorWorldStart =
		 * DisplaySystem.getDisplaySystem().getWorldCoordinates(new
		 * Vector2f(monitorScreen.getLocalTranslation().x,
		 * monitorScreen.getLocalTranslation().y), 0.9f); if(cursorWorldStart !=
		 * null && manipulatabledOjbect != null){ FloatBuffer vectorBuff =
		 * BufferUtils.createVector3Buffer(2); FloatBuffer colorBuff =
		 * BufferUtils.createFloatBuffer(new ColorRGBA[]{ColorRGBA.gray,
		 * ColorRGBA.white}); BufferUtils.setInBuffer(cursorWorldStart,
		 * vectorBuff, 0);
		 * BufferUtils.setInBuffer(manipulatabledOjbect.getLocalTranslation(),
		 * vectorBuff, 1); line.setLineWidth(1f); line.reconstruct(vectorBuff,
		 * null, colorBuff, null); line.updateRenderState();
