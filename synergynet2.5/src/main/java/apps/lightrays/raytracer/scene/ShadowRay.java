...
public class ShadowRay extends Ray {
	 ...
	 * (non-Javadoc)
	 * @see
	 * apps.lightrays.raytracer.scene.Ray#processHit(apps.lightrays.raytracer
	 * .scene.RayHitInfo)
	 */
	protected void processHit(RayHitInfo hit) {
		if ((hit.distance < closest_hit.distance)
				&& (hit.distance > Vector.EPSILON)) {
	 ...
	 * (non-Javadoc)
	 * @see
	 * apps.lightrays.raytracer.scene.Ray#shade(apps.lightrays.raytracer.scene
	 * .Colour)
	 */
	public void shade(Colour c) {
		if ((closest_hit.object != null)
				&& closest_hit.object.getOpticProperties().luminous) {
