...
public class ButtonRenderer {
	 ...
	 *            the pressed
	 */
	public static void RenderButton(Frame button, String text, int buttonWidth,
			boolean pressed) {
		...
		if (text.equals("s")) {
			buttonLength = (buttonWidth * 2) + 6;
		}

		if (pressed) {
			gfx.setColor(Color.black);
			gfx.drawLine(1, 1, buttonLength, 1);
			gfx.drawLine(1, 1, 1, buttonWidth);
		...
		((Spatial) (button.getImplementationObject())).updateGeometricState(0f,
				false);
		((GraphicsImageQuad) (button.getImplementationObject()))
				.updateGraphics();
	}
