...
public class DateTextWritter {
	 ...
	 *            the corrections
	 */
	public void addRecord(String target, String input, int result,
			double effectiveTime, int corrections) {

		try {
			FileWriter out = new FileWriter(fileName, true);
			BufferedWriter writer = new BufferedWriter(out);
			writer.write(target + " " + input + " " + result + " "
					+ effectiveTime + " " + corrections + "\n");
			writer.close();
		} catch (IOException e) {
			e.printStackTrace();
		}
	...
	public String createFile(String device, String type) {
		...
		String dateString = currentDate.getMonth() + 1 + "-"
				+ currentDate.getDate() + " [" + currentDate.getHours() + "~"
				+ currentDate.getMinutes() + "~" + currentDate.getSeconds()
				+ "]";
		fileName = "log//3DExperimentLog/Excel/" + dateString + " (" + device
				+ ").txt";
