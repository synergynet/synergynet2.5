...
public class RoundButton extends Node {
	 ...
	 *            the text texture
	 */
	public RoundButton(String name, float radius, float height, URL bgTexture,
			URL textTexture) {
		super(name);
		this.keyName = name;
		this.radius = radius;
	...
	private Disk createTextLabel() {
		...
		disk.updateRenderState();
		disk.getLocalTranslation().z = disk.getLocalTranslation().z
				+ ((this.height + 1) / 2) + 0.71f;
		disk.setModelBound(new BoundingBox());
		disk.updateModelBound();

		MultiTouchButtonPress ButtonTopPress = new MultiTouchButtonPress(disk,
				buttonNode);
