...
public class ButtonNode extends Node {
	 ...
	 * @param textTexture
	 *            the text texture
	 * @param highlightedTextureURL
	 *            the highlighted texture url
	 */
	public ButtonNode(String name, float width, float length, float height,
			float slope, URL bgTexture, URL textTexture,
			URL highlightedTextureURL) {
		super(name);
		this.keyName = name;
		this.width = width;
		this.length = length;
	...
	private Quad createTextQuad() {
		...
		final Quad quad = new Quad("button " + this.name, this.width
				* (1.5f + scaleOffSet), this.length * (1.5f + scaleOffSet));
		
		final Quad highlightedQuad = new Quad(
				"highlighted button " + this.name, this.width * (1.9f),
				this.length * (1.8f));
		...
		texture = TextureManager.loadTexture(textTextureURL,
		...
		if (highlightedTextureURL != null) {
			...
			tsHighlighted = DisplaySystem.getDisplaySystem().getRenderer()
					.createTextureState();
			tsHighlighted
					.setCorrectionType(TextureState.CorrectionType.Perspective);
			textureHighlighted = TextureManager.loadTexture(
					highlightedTextureURL,
					Texture.MinificationFilter.Trilinear,
					Texture.MagnificationFilter.Bilinear);
		...
		if (zvalueOfButtonLabel != 0) {
			quad.getLocalTranslation().z = rb.getLocalTranslation().z
					+ ((this.height + 1) / 2) + zvalueOfButtonLabel;
			highlightedQuad.getLocalTranslation().z = rb.getLocalTranslation().z
					+ ((this.height + 1) / 2) + zvalueOfButtonLabel;
		} else {
			quad.getLocalTranslation().z = rb.getLocalTranslation().z
					+ ((this.height + 1) / 2) + 0.1f;
			highlightedQuad.getLocalTranslation().z = rb.getLocalTranslation().z
					+ ((this.height + 1) / 2) + 0.1f;
		}
		...
		ButtonTopPress.addButtonListener(new FreeButtonListener() {
			...
			public void buttonPressed() {
				...
				} else if (self.feedbackMode
						.equals(self.FEEDBACK_MODE_COLORHIGHLIGHTED)) {
	 ...
	 *
	 * @param b
	 *            the new button body visability
	 */
