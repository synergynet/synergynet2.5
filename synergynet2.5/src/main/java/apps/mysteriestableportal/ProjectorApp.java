	...
	public void announce() {
		try {
			RapidNetworkManager.getTableCommsClientService().sendMessage(
					new AnnounceProjectorMessage(ControllerApp.class));
		} catch (IOException e) {
		...
		RapidNetworkManager.registerMessageProcessor(new MessageProcessor() {
			...
			public void process(Object obj) {
				if (obj instanceof SearchProjectorsMessage) {
					...
					frame.setHeight(DisplaySystem.getDisplaySystem()
							.getHeight());
					...
					TableIdentity sourceTableId = ((ProjectorPostItemsMessage) obj)
							.getSourceTableId();
					...
					List<ContentItem> items = ((ProjectorPostItemsMessage) obj)
							.getItems();
					for (ContentItem item : items) {
						if (contentSystem.getAllContentItems().containsKey(
								item.getName())) {
							return;
						}
					...
					frame.setLocation(DisplaySystem.getDisplaySystem()
