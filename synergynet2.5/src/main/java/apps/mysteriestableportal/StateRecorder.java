...
public class StateRecorder {
	...
	private String restoreFileName = "restore"
	...
	public StateRecorder(DefaultSynergyNetApp app) {
		restoreFolder = new File(app.getApplicationDataDirectory(), "restore"
				+ app.getName());
			...
			while (line != null) {
				...
				if (line.startsWith("#")) {
					...
					if (mysteryPath
						...
						items.addAll(new DinnerDisasterBuilder()
								.build(contentSystem));
					} else {
						items.addAll(contentSystem
								.loadContentItems(mysteryPath));
					}
				} else {
	 ...
	 *            the items
	 */
	public void registerMysteryContentItems(final String mysteryPath,
