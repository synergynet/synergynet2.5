...
public class TwinObject {
	 ...
	 *            the init position
	 */
	public TwinObject(String name, ContentSystem contentSystem, Node worldNode,
			Spatial manipulatedOjbect, Spatial twinObject, Vector3f initPosition) {
		...
		twinObject.setLocalTranslation(initPosition.x, initPosition.y,
				initPosition.z);
		...
		if ((twinObject != null) && (manipulatedOjbect != null)) {
			...
			FloatBuffer colorBuff = BufferUtils
					.createFloatBuffer(new ColorRGBA[] { ColorRGBA.gray,
							ColorRGBA.white });
			BufferUtils.setInBuffer(twinObject.getLocalTranslation(),
					vectorBuff, 0);
			BufferUtils.setInBuffer(manipulatedOjbect.getLocalTranslation(),
					vectorBuff, 1);
