...
public class Telescope {
	 ...
	 * @param initTelescopePosition
	 *            the init telescope position
	 * @param initTelescopeZoom
	 *            the init telescope zoom
	 * @param telescopeRadius
	 *            the telescope radius
	 */
	public Telescope(String name, ContentSystem contentSystem, Node worldNode,
			Node orthoNode, List<Spatial> manipulatableOjbects,
			Vector3f mainCameraPosition, Vector2f initTelescopePosition,
			float initTelescopeZoom, float telescopeRadius) {
		...
		telescopeLen = new TelescopeLen(name + "telescopeLen", contentSystem,
				telescopeRadius, camNode, manipulatableOjbects);
		orthoNode.attachChild(telescopeLen);
		telescopeLen.setLocalTranslation(initTelescopePosition.x,
				initTelescopePosition.y, 0);
		...
		Vector3f cursorWorldStart = DisplaySystem.getDisplaySystem()
				.getWorldCoordinates(initTelescopePosition, 0);
		Vector3f cursorWorldEnd = DisplaySystem.getDisplaySystem()
				.getWorldCoordinates(initTelescopePosition, 1);
