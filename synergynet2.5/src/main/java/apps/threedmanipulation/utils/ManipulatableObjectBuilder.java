...
public class ManipulatableObjectBuilder {
	 ...
	 * @param clonedObject
	 *            the cloned object
	 * @param isOneFingerMove
	 *            the is one finger move
	 * @param isManipulatedOjbectNode
	 *            the is manipulated ojbect node
	 */
	public static void buildManipulatedObject(final Context context,
			final Spatial object, final Spatial clonedObject,
			boolean isOneFingerMove, boolean isManipulatedOjbectNode) {
		...
		if (isManipulatedOjbectNode) {
			twinObjectRotateTranslate = new ObjectRotateTranslate(
					((Node) clonedObject).getChild(0), clonedObject, object);
		} else {
			twinObjectRotateTranslate = new ObjectRotateTranslate(clonedObject,
					clonedObject, object);
		}
		twinObjectRotateTranslate.setPickMeOnly(true);
		twinObjectRotateTranslate
				.addRotateTranslateScaleListener(new ObjectRotateTranslateScaleListener() {
					@Override
					public void itemMoved(Spatial targetSpatial,
						...
						if (twinObjects.keySet().contains(object.getName())) {
							TwinObject twinObject = twinObjects.get(object
									.getName());
		...
		if (isOneFingerMove) {
			...
			if (isManipulatedOjbectNode) {
				cprts1 = new ControlPointRotateTranslateScale(
						((Node) object).getChild(0), object);
			} else {
			...
			cprts1.addRotateTranslateScaleListener(new RotateTranslateScaleListener() {
				...
				public void itemMoved(Spatial targetSpatial,
					...
					if (context.getIndirectManipulationMode()
							.equals("touchpad")) {
					...
					} else if (context.getIndirectManipulationMode().equals(
							"twinobject")) {
						if (twinObjects.keySet().contains(object.getName())) {
			...
			if (!isManipulatedOjbectNode) {
				...
				cprts1 = new ObjectRotateTranslate(((Node) object).getChild(0),
						object);
			}
			cprts1.setPickMeOnly(true);
		...
		if (!isManipulatedOjbectNode) {
			...
			objectDoubleClick = new ObjectRightClick(
					((Node) object).getChild(0), object);
		}
		objectDoubleClick.addMultiTouchListener(new CursorEventListener() {
			...
			public void cursorRightClicked(ScreenCursor c,
				...
				if (context.getIndirectManipulationMode().equals("touchpad")) {
					if (!touchPads.keySet().contains(object.getName())) {
						TouchPad touchPad = new TouchPad(UUID.randomUUID()
								.toString() + "touchPad", context
								.getContentSystem(), context.getWorldNode(),
								context.getOrthoNode(), 150, object,
						...
						"twinobject")) {
					if (!twinObjects.keySet().contains(object.getName())) {
						TwinObject twinObject = new TwinObject(UUID
								.randomUUID().toString() + "twinObject",
								context.getContentSystem(), context
										.getWorldNode(), object, clonedObject,
								new Vector3f(10, 5, 110));
