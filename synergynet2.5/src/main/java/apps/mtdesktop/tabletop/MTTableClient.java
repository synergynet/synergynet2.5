...
public class MTTableClient extends DefaultSynergyNetApp {
	...
	public void addContent() {
		...
		background = (LightImageLabel) contentSystem
				.createContentItem(LightImageLabel.class);
		background.drawImage(MTDesktopConfigurations.class
				.getResource("tabletop/wood-texture.jpg"));
		...
		/*
		 * MTKeyboard keyboard = (MTKeyboard)
		 * contentSystem.createContentItem(MTKeyboard.class);
		 * keyboard.addKeyListener(new MTKeyListener(){
		 * @Override public void keyPressedEvent(KeyEvent evt) {
		 * System.out.println(evt.getKeyChar()); }
		 * @Override public void keyReleasedEvent(KeyEvent evt) { } });
		 * keyboard.setScale(0.4f); Window w = (Window)
		 * contentSystem.createContentItem(Window.class);
		 * w.addSubItem(keyboard); w.centerItem();
		 */

		final Window w = (Window) contentSystem.createContentItem(Window.class);
		w.getBackgroundFrame().setVisible(false, false);

		SimpleButton b1 = (SimpleButton) contentSystem
				.createContentItem(SimpleButton.class);
		b1.setAutoFitSize(false);
		b1.setWidth(30);
		b1.setHeight(30);
		b1.drawImage(MTDesktopConfigurations.class
				.getResource("tabletop/text.png"));

		SimpleButton b2 = (SimpleButton) contentSystem
				.createContentItem(SimpleButton.class);
		b2.setAutoFitSize(false);
		b2.setWidth(30);
		b2.setHeight(30);
		b2.drawImage(MTDesktopConfigurations.class
				.getResource("tabletop/link.gif"));
		...
		final MultiLineTextLabel mttl = (MultiLineTextLabel) contentSystem
		...
		final LineItem line = (LineItem) contentSystem
	...
	public void announce() {
		try {
			String fileServerUrl = "http://"
					+ InetAddress.getLocalHost().getHostAddress() + ":"
					+ MTDesktopConfigurations.SERVER_PORT;
			for (Class<?> targetClass : RapidNetworkManager
					.getReceiverClasses()) {
				RapidNetworkManager.getTableCommsClientService().sendMessage(
						new AnnounceTableMessage(targetClass, fileServerUrl));
			}
		} catch (IOException e) {
			e.printStackTrace();
		}
	...
	public void onActivate() {
		...
		RapidNetworkManager.registerMessageProcessor(new MessageProcessor() {
			...
			public void process(Object obj) {
				if (obj instanceof UnicastSearchTableMessage) {
					...
					if (manager.getBasketManager().getBaskets()
							.containsKey(msg.getSender())) {
						manager.getBasketManager().unregisterBasket(
								msg.getSender());
						manager.unregisterMouseCursor(msg.getSender());
						manager.getPad().unregisterUser(msg.getSender());
					}
					JmeNetworkedBasket basket = manager.getBasketManager()
							.registerBasketForPeer(msg.getSender(),
									msg.getClientPosition());
					if (basket != null) {
						manager.getPad().registerUser(basket.getTableId(),
								basket.getWindow().getBackgroundColour());
		...
		if (!RapidNetworkManager.getReceiverClasses().contains(
				DesktopClient.class)) {
			RapidNetworkManager.getReceiverClasses().add(DesktopClient.class);
		}
