...
public class JmeNetworkedBasket {
	...
	/** The hight. */
	protected int width = (int) (0.6 * DisplaySystem.getDisplaySystem()
			.getWidth()), hight = (int) (0.6 * DisplaySystem.getDisplaySystem()
			.getHeight());
	...
	public JmeNetworkedBasket(ContentSystem contentSystem) {
		...
		window.setBackgroundColour(Color.getHSBColor(
		...
		// label.setLocalLocation((label.getWidth() - window.getWidth())/2,
		// (window.getHeight() - label.getHeight())/2) ;
		// window.addSubItem(label);

		bin = (LightImageLabel) contentSystem
				.createContentItem(LightImageLabel.class);
		bin.setAutoFitSize(false);
		bin.drawImage(MTDesktopConfigurations.class
				.getResource("tabletop/bin.png"));
		bin.setWidth(180);
		bin.setHeight(180);
		bin.setLocalLocation(-window.getWidth() + bin.getWidth(), 150);

		copy = (LightImageLabel) contentSystem
				.createContentItem(LightImageLabel.class);
		copy.setAutoFitSize(false);
		copy.drawImage(MTDesktopConfigurations.class
				.getResource("tabletop/copy.png"));
		copy.setWidth(180);
		copy.setHeight(180);
		...
		((Spatial) window.getBackgroundFrame().getImplementationObject())
				.setZOrder(MTTableClient.background.getOrder() + 1);
	...
	public void detachItem(ContentItem item) {
		...
		for (ContentItem i : window.getAllItemsIncludeSystemItems()) {
			((OrthoContentItem) i).setRotateTranslateScalable(false);
		}
	}
	...
	/*
	 * public TextLabel getTextLabel(){ return label; }
	 */
	/**
	 * Gets the position.
	 *
	...
	public boolean isBasketComponent(String spatialName) {
		if (this.getWindow().getName().equals(spatialName)
				|| this.getWindow().getBackgroundFrame().getName()
						.equals(spatialName)
				|| /* this.getTextLabel().getName().equals(spatialName) || */this.bin
						.getName().equals(spatialName)
				|| this.copy.getName().equals(spatialName)) {
			return true;
		} else {
			return false;
		}
	...
	public void linkItem(ContentItem item) {
		...
		final LineItem line = (LineItem) item.getContentSystem()
	...
	public void setTableId(TableIdentity tableId) {
		...
		if (idLabel.length() > 10) {
			idLabel = idLabel.substring(0, 10) + "..";
			// label.setText(idLabel);
			// label.setLocalLocation(0 , window.getHeight()/2-
			// label.getHeight()) ;
		}
	}
