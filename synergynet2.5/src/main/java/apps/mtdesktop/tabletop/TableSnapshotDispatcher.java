...
public class TableSnapshotDispatcher {
	...
	public TableSnapshotDispatcher() {
		...
		at = AffineTransform.getScaleInstance(
				MTDesktopConfigurations.defaultRadarImageScale,
				MTDesktopConfigurations.defaultRadarImageScale);
		img = new BufferedImage(width, height, BufferedImage.TYPE_INT_RGB);
		dest = new BufferedImage(
				(int) (width * MTDesktopConfigurations.defaultRadarImageScale),
				(int) (height * MTDesktopConfigurations.defaultRadarImageScale),
			...
			try {
				if (DisplaySystem.getDisplaySystem().getRenderer() != null) {
					DisplaySystem
							.getDisplaySystem()
							.getRenderer()
							.grabScreenContents(buff, Image.Format.RGB4, 0, 0,
									width, height);
					final int w = width;
					final int h = height;
					Thread saveThread = new Thread() {
						...
						public void run() {
							try {
								...
								ImageIO.write(dest, "gif", new File(
										MTDesktopConfigurations.OutboxFolder
