...
public class MouseCursor implements DesktopMouseListener {
	...
	/** The cursor shift x. */
	private int cursorShiftX = -CURSOR_WIDTH / 2;
	...
	/*
	 * (non-Javadoc)
	 * @see apps.mtdesktop.tabletop.TabletopContentManager.DesktopMouseListener#
	 * mouseClicked
	 * (synergynetframework.appsystem.services.net.localpresence.TableIdentity,
	 * apps.mtdesktop.messages.util.MouseEventInfo)
	 */
	...
	public void mouseDragged(TableIdentity tableId, MouseEventInfo evt) {
		if ((this.tableId == null) || !this.tableId.equals(tableId)) {
			return;
		}
		Point.Float tablePos = MultiTouchCursorSystem
				.screenToTable(new Vector2f(evt.getX() + cursorShiftX, evt
						.getY() + cursorShiftY));
		MultiTouchCursorEvent e = new MultiTouchCursorEvent(cursorId,
				new Point.Float(tablePos.x, tablePos.y),
				new Point2D.Float(0, 0), 0, 0);
