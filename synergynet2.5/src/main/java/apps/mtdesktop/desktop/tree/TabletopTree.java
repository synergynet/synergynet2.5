...
public class TabletopTree extends JTree implements KeyListener, MouseListener {
	 ...
	 * <code>addDataTransferListener<code> method. When
	 ...
	 *            the new model
	 */
	public TabletopTree(TreeModel newModel) {
		super(newModel);
	}
	 ...
	 *            the root
	 */
	public TabletopTree(TreeNode root) {
		super(root);
	}
	 ...
	 *            the asks
	 */
	public TabletopTree(TreeNode root, boolean asks) {
		super(root, asks);
	}
	...
	public InboxNode getInboxNode(TableIdentity tableId) {
		for (int i = 0; i < top.getChildCount(); i++) {
			if ((top.getChildAt(i) != null)
					&& (top.getChildAt(i) instanceof InboxNode)
					&& ((InboxNode) top.getChildAt(i)).getPeerData()
	...
	public OutboxNode getOutboxNode(TableIdentity tableId) {
		for (int i = 0; i < top.getChildCount(); i++) {
			if ((top.getChildAt(i) != null)
					&& (top.getChildAt(i) instanceof OutboxNode)
					&& ((OutboxNode) top.getChildAt(i)).getPeerData()
							.getTableId().equals(tableId)) {
	...
	public void mousePressed(MouseEvent e) {
		if (e.isPopupTrigger()) {
			if ((this.getSelectionPaths() == null)
					|| (this.getSelectionPaths().length == 0)) {
				return;
			}
			DefaultMutableTreeNode selectedNode = (DefaultMutableTreeNode) this
					.getSelectionPath().getLastPathComponent();
	...
	public boolean registerTable(PeerData peerData) {
		if ((this.getOutboxNode(peerData.getTableId()) != null)
				&& (this.getInboxNode(peerData.getTableId()) != null)) {
			return false;
		}
