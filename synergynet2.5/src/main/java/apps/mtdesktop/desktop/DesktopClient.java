...
public class DesktopClient extends DefaultSynergyNetApp {
	 ...
	 * <code>addmtdesktopNetworkListener<code> method. When
					...
					try {
						while (!status.equals(ConnectionStatus.CONNECTED_TO_TABLE)) {
							RapidNetworkManager
								...
								.sendMessage(
										new UnicastSearchTableMessage(
												MTTableClient.class,
												MTDesktopConfigurations.tableId,
												currentPosition));
	 ...
	 *            the arguments
		...
		public static void main(String args[]) {
			...
			try {
				info = new ApplicationInfo(DesktopClient.class.getName(),
					"Desktop Client", "version1.0",
	 ...
	 *            the info
		...
		desktopFrame = new DesktopFrame(this);
			...
			@Override
				public void process(Object obj) {
					if (obj instanceof AnnounceTableMessage) {
						if (!(((TableMessage) obj).getSender()
							.equals(MTDesktopConfigurations.tableId))) {
							return;
						}
						tableId = ((TableMessage) obj).getSender();
						File_SERVER_URL = ((AnnounceTableMessage) obj)
							.getFileServerUrl();
						synchronized (this) {
			...
			RapidNetworkManager
				.registerMessageProcessor(new DesktopInboxContentManager(
						desktopFrame.getTree()));
			RapidNetworkManager.registerMessageProcessor(new KeyboardRedirector(
				desktopFrame));
			if (!RapidNetworkManager.getReceiverClasses().contains(
				MTTableClient.class)) {
				RapidNetworkManager.getReceiverClasses().add(MTTableClient.class);
			}
