 ...
 */
public class DesktopFrame extends JFrame implements mtdesktopNetworkListener,
		FileTransferListener, ControlPanelListener {
		...
		menuItem3.addItemListener(new ItemListener() {
			...
			@Override
				public void itemStateChanged(ItemEvent e) {
					ShowMultiPadMessage msg = new ShowMultiPadMessage(
	...
	/*
	 * (non-Javadoc)
	 * @see apps.mtdesktop.desktop.DesktopClient.mtdesktopNetworkListener#
	 * connectionUpdate(apps.mtdesktop.desktop.DesktopClient.ConnectionStatus)
	 */
	@Override
	 ...
	 * (non-Javadoc)
	 * @see
	 * apps.mtdesktop.desktop.ControlPanel.ControlPanelListener#deletePressed()
	 */
	@Override
	 ...
	 * (non-Javadoc)
	 * @see
	 * apps.mtdesktop.desktop.ControlPanel.ControlPanelListener#shareDesktopPressed
	 * (boolean)
	 */
	@Override
	public void shareDesktopPressed(boolean isEnabled) {
		try {
			if (DesktopClient.tableId != null) {
				...
				RapidNetworkManager.getTableCommsClientService()
						.sendMessage(
								new LaunchVncMessage(MTTableClient.class,
										DesktopClient.tableId, UUID
												.randomUUID().toString(),
										ipAddress,
										MTDesktopConfigurations.vncPort,
										MTDesktopConfigurations.vncPassword,
										isEnabled));
			}
		} catch (Exception exp) {
	 ...
	 * (non-Javadoc)
	 * @see
	 * apps.mtdesktop.desktop.DesktopClient.mtdesktopNetworkListener#tableConnected
	...
	public void tableConnected(TableIdentity tableId) {
		...
		if (shortTableName.length() > 7) {
			shortTableName = shortTableName.substring(0, 7) + "..";
		}
