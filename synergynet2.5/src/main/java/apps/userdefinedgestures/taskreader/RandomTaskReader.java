...
public class RandomTaskReader implements TaskReader {
	...
	private List<String> getSequenceTaskList() {
		...
		String[] transformer = { "moveright", "moveup", "moveinside",
				"moverightinside", "moverightoutside", "rotatex", "rotatey",
				"rotatez", "scaleup", "scaledown" };
		...
		for (int i = 0; i < objects.length; i++) {
			for (int j = 0; j < viewports.length; j++) {
				for (int z = 0; z < transformer.length; z++) {
					sequenceTaskList.add(objects[i] + "," + viewports[j] + ","
							+ transformer[z]);
				}
			}
