...
public class Dial {
	 ...
	 * @param startValue
	 *            the start value
	 * @param valuePerFullRotation
	 *            the value per full rotation
	 * @param measure
	 *            the measure
	 */
	public Dial(ContentSystem contentSystem, int xLoc, int yLoc,
			float dialRadius, float startValue, float valuePerFullRotation,
			String measure) {
		...
		bg = (RoundImageLabel) contentSystem
		...
		distanceText.setText(new DecimalFormat("0.##").format(result)
				+ measurement);
		distanceText.setLocation(
				bg.getLocation().x + radius + (distanceText.getWidth() / 2)
						+ gap, bg.getLocation().y);
		...
		ItemEventAdapter ie = new ItemEventAdapter() {
			...
			private Vector2f origin = new Vector2f(bg.getLocation().x,
					bg.getLocation().y);
			...
			public void cursorChanged(ContentItem item, long id, float x,
				...
				if (down) {
					Vector2f touchDrag = new Vector2f(x
							* DisplaySystem.getDisplaySystem().getRenderer()
									.getWidth(), y
							* DisplaySystem.getDisplaySystem().getRenderer()
									.getHeight());

					angle = new Vector2f(touchDrag.x - origin.x, touchDrag.y
							- origin.y).angleBetween(new Vector2f(touchDown.x
							- origin.x, touchDown.y - origin.y));
					...
					if (changeInAngle > (FastMath.DEG_TO_RAD * 180)) {
						changeInAngle = -(((FastMath.DEG_TO_RAD * 180) - FastMath
								.abs(lastAngle)) + ((FastMath.DEG_TO_RAD * 180) - FastMath
								.abs(angle)));
					} else if (changeInAngle < (FastMath.DEG_TO_RAD * -180)) {
						changeInAngle = ((FastMath.DEG_TO_RAD * 180) - FastMath
								.abs(lastAngle))
								+ ((FastMath.DEG_TO_RAD * 180) - FastMath
										.abs(angle));
					}
					...
					distanceText.setText(new DecimalFormat("0.##")
							.format(toDisplay) + measurement);
					distanceText.setLocation(bg.getLocation().x + radius
							+ (distanceText.getWidth() / 2) + gap,
							bg.getLocation().y);
					marker.setAsTopObject();
	...
	public void addPrecisionSlider(float range) {
		...
		sliderBox = (ImageTextLabel) contentSystem
				.createContentItem(ImageTextLabel.class);
		sliderBox.setLocation(bg.getLocation().x + radius + gap,
				bg.getLocation().y);
		...
		markerMove.setSourceLocation(new Location(sliderBox.getLocation().x
				- (sliderBox.getWidth() / 2), sliderBox.getLocation().y, 0));
		markerMove.setTargetLocation(new Location(sliderBox.getLocation().x
				+ (sliderBox.getWidth() / 2), sliderBox.getLocation().y, 0));
		...
		markerMove.addItemListener(new ItemEventAdapter() {
			...
			public void cursorChanged(ContentItem item, long id, float x,
				...
				if ((y > (sliderBox.getLocation().y - (sliderBox.getHeight() / 2)))
						&& (y < (sliderBox.getLocation().y + (sliderBox
								.getHeight() / 2)))) {
					
					if (upperLimit) {
						if ((result + ((y - sliderBox.getLocation().y) * (rangeValue / sliderBox
								.getHeight()))) > upperBound) {
							return;
						}
					}
					...
					if (lowerLimit) {
						if ((result + ((y - sliderBox.getLocation().y) * (rangeValue / sliderBox
								.getHeight()))) < lowerBound) {
							return;
						}
					}

					markerMove.setSourceLocation(new Location(sliderBox
							.getLocation().x - (sliderBox.getWidth() / 2), y, 0));
					markerMove.setTargetLocation(new Location(sliderBox
							.getLocation().x + (sliderBox.getWidth() / 2), y, 0));
					markerMove.setAsTopObject();

					preciseValue = (markerMove.getTargetLocation().y - sliderBox
							.getLocation().y)
