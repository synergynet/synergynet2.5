...
public class Slider {
	 ...
	 *            the suffix
	 */
	public Slider(ContentSystem contentSystem, int x, int y, int widthNew,
			int heightNew, int maxValue, int minValue, String prefix,
			String suffix) {
		this.locX = x;
		this.locY = y;
		...
		markerOne.setSourceLocation(new Location(x - (width / 2), y
				+ (height / 2), 0));
		markerOne.setTargetLocation(new Location(x - (width / 2), y
				- (height / 2), 0));
		...
		markerDistance = (LineItem) contentSystem
				.createContentItem(LineItem.class);
		markerDistance.setSourceLocation(new Location(x - (width / 2), y
				+ (height / 2), 0));
		...
		markerTwo.addItemListener(new ItemListener() {
			...
			public void cursorChanged(ContentItem item, long id, float x,
				...
				if ((x > (locX - (width / 2))) && (x < (locX + (width / 2)))) {
					markerTwo.setSourceLocation(new Location(x, locY
							+ (height / 2), 0));
					markerTwo.setTargetLocation(new Location(x, locY
							- (height / 2), 0));
					markerDistance.setTargetLocation(new Location(x, locY
							+ (height / 2), 0));
					markerTwo.setAsTopObject();
					markerDistance.setAsTopObject();
					result = (((markerTwo.getSourceLocation().x - markerOne
							.getSourceLocation().x) + min) * (max - min))
							/ width;
					setText();
		...
		result = (((markerTwo.getSourceLocation().x - markerOne
				.getSourceLocation().x) + min) * (max - min)) / width;
		distanceText.setLocalLocation(locX,
				y + (height / 2) + (distanceText.getHeight() / 2) + 10);
		prefixText.setLocalLocation(distanceText.getLocation().x
				- (distanceText.getWidth() / 2) - (prefixText.getWidth() / 2),
				distanceText.getLocation().y);
		suffixText.setLocalLocation(distanceText.getLocation().x
				+ (distanceText.getWidth() / 2) + (suffixText.getWidth() / 2),
				locY + (height / 2) + (distanceText.getHeight() / 2) + 10);
	}
	...
	private void setText() {
		...
		distanceText.setLocalLocation(
				prefixText.getLocation().x + (prefixText.getWidth() / 2)
						+ (distanceText.getWidth() / 2), locY + (height / 2)
						+ (distanceText.getHeight() / 2) + 10);
		suffixText.setLocalLocation(distanceText.getLocation().x
				+ (distanceText.getWidth() / 2) + (suffixText.getWidth() / 2),
				locY + (height / 2) + (distanceText.getHeight() / 2) + 10);
	}
	...
	public void setValue(float value) {
		result = value;
		float newMarkerXPos = ((result / (max - min)) * width)
				+ markerOne.getSourceLocation().x;
		markerTwo.setSourceLocation(new Location(newMarkerXPos, markerTwo
				.getSourceLocation().y, 0));
		markerTwo.setTargetLocation(new Location(newMarkerXPos, markerTwo
				.getTargetLocation().y, 0));
		markerDistance.setTargetLocation(new Location(newMarkerXPos,
				markerDistance.getTargetLocation().y, 0));
		setText();
		markerTwo.setAsTopObject();
	}
