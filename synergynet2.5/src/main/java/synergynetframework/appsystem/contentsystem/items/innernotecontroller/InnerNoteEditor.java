...
public class InnerNoteEditor implements Updateable {
	...
	/** The Constant log. */
	private static final Logger log = Logger.getLogger(InnerNoteEditor.class
			.getName());
	 ...
	 *            the editted item
	 */
	protected InnerNoteEditor(InnerNoteController noteController,
			QuadContentItem edittedItem) {
		...
		noteNode = (OrthoContainer) contentSystem
				.createContentItem(OrthoContainer.class);

		// set note label
		noteLabel = (MultiLineTextLabel) contentSystem
	 ...
	 * @param edittedItem
	 *            the editted item
	 * @param edittedParentItem
	 *            the editted parent item
	 */
	protected InnerNoteEditor(InnerNoteController noteController,
			QuadContentItem edittedItem, ContentItem edittedParentItem) {
	...
	private void addEditButton() {
		...
		editButton = (SimpleButton) contentSystem
		...
		editButton.drawImage(GraphConfig.nodeEditImageResource, 0, 0,
				editButton.getWidth(), editButton.getHeight());
		...
		editButton.setLocalLocation(
				(noteLabel.getWidth() / 2) - (editButton.getWidth() / 2),
				(noteLabel.getHeight() / 2) - (editButton.getHeight() / 2));
	...
	public void initNoteNode() {
		...
		if (edittedParentItem != null) {
			item = edittedParentItem;
			noteNode.setLocalLocation(item.getLocalLocation().getX()
					+ edittedItem.getLocalLocation().getX(), item
					.getLocalLocation().getY()
					+ edittedItem.getLocalLocation().getY());
			noteLabel.setHeight(noteLabel.getHeight()
					+ (edittedParentItem.getBorderSize() * 2));
			noteLabel.setWidth(noteLabel.getWidth()
					+ (edittedParentItem.getBorderSize() * 2));
		} else {
			item = edittedItem;
			noteNode.setLocalLocation(item.getLocalLocation().getX(), item
					.getLocalLocation().getY());
		}
	...
	private void listenToChanges() {
		...
		noteNode.addOrthoControlPointRotateTranslateScaleListener(new OrthoControlPointRotateTranslateScaleAdapter() {
			...
			public void itemScaled(ContentItem item, float newScaleFactor,
				...
				for (InnerNoteEventListener l : innerNoteEventListeners) {
					if (edittedParentItem != null) {
						l.noteScaled(edittedParentItem, newScaleFactor,
								oldScaleFactor);
					} else {
						l.noteScaled(edittedItem, newScaleFactor,
								oldScaleFactor);
					}
				}
			}
			...
			public void itemTranslated(ContentItem item, float newLocationX,
				...
				if (edittedParentItem != null) {
					edittedParentItem.setLocalLocation(newLocationX,
							newLocationY);
				} else {
				...
				for (InnerNoteEventListener l : innerNoteEventListeners) {
					if (edittedParentItem != null) {
						l.noteTranslated(edittedParentItem, newLocationX,
								newLocationY, oldLocationX, oldLocationY);
					} else {
						l.noteTranslated(edittedItem, newLocationX,
								newLocationY, oldLocationX, oldLocationY);
					}
				}
			}
		...
		noteLabel.addItemListener(new ItemEventAdapter() {
			...
			public void cursorClicked(ContentItem item, long id, float x,
				...
				Rectangle center = new Rectangle(contentItem.getWidth() - 20,
						contentItem.getHeight() - 20, contentItem.getWidth(),
						contentItem.getHeight());
	...
	private void showKeyborad() {
		...
		keyboard.setLocalLocation(noteNode.getLocalLocation().getX(), noteNode
				.getLocalLocation().getY());
