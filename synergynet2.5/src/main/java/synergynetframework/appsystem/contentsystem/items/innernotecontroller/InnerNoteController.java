...
public class InnerNoteController implements InnerNoteEventListener {
		...
		for (final ContentItem ci : items) {
			...
			quadItem.addItemListener(new ItemEventAdapter() {
				...
				public void cursorClicked(ContentItem item, long id, float x,
					...
					Rectangle rightBottomCorner = new Rectangle(contentItem
							.getWidth() - 20, contentItem.getHeight() - 20,
							contentItem.getWidth(), contentItem.getHeight());
	 ...
	 *            the main sub item
	 */
	public void addNoteController(final ContentItem parentItem,
			final QuadContentItem mainSubItem) {
		...
		mainSubItem.addItemListener(new ItemEventAdapter() {
			...
			public void cursorDoubleClicked(ContentItem item, long id, float x,
					float y, float pressure) {
				super.cursorClicked(item, id, x, y, pressure);
				Rectangle rightBottomCorner = new Rectangle(mainSubItem
						.getWidth() - 40, mainSubItem.getHeight() - 40,
						mainSubItem.getWidth(), mainSubItem.getHeight());
	 ...
	 * #noteBringToTop(synergynetframework.appsystem.contentsystem
	 * .items.ContentItem)
	 */
	@Override
	 ...
	 * synergynetframework.appsystem.contentsystem.items.innernotecontroller
	 * .InnerNoteEventListener
	 * #noteChanged(synergynetframework.appsystem.contentsystem
	 * .items.ContentItem, java.lang.String)
	 */
	@Override
	 ...
	 * synergynetframework.appsystem.contentsystem.items.innernotecontroller
	 * .InnerNoteEventListener
	 * #noteLabelOn(synergynetframework.appsystem.contentsystem
	 * .items.ContentItem, boolean)
	 */
	@Override
	 ...
	 * synergynetframework.appsystem.contentsystem.items.innernotecontroller
	 * .InnerNoteEventListener
	 * #noteRotated(synergynetframework.appsystem.contentsystem
	 * .items.ContentItem, float, float)
	 */
	@Override
	 ...
	 * synergynetframework.appsystem.contentsystem.items.innernotecontroller
	 * .InnerNoteEventListener
	 * #noteScaled(synergynetframework.appsystem.contentsystem
	 * .items.ContentItem, float, float)
	 */
	@Override
	 ...
	 * synergynetframework.appsystem.contentsystem.items.innernotecontroller
	 * .InnerNoteEventListener
	 * #noteTranslated(synergynetframework.appsystem.contentsystem
	 * .items.ContentItem, float, float, float, float)
	 */
	@Override
	public void noteTranslated(ContentItem edittedItem, float newLocationX,
		...
		for (InnerNoteEventListener l : innerNoteEventListeners) {
			l.noteTranslated(edittedItem, newLocationX, newLocationY,
					oldLocationX, oldLocationY);
		}
