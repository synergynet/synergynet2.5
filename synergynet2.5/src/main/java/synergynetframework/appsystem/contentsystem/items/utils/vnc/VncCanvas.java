...
public class VncCanvas extends Canvas implements KeyListener, MouseListener,
	...
	/**
	 * The listener interface for receiving vncImage events. The class that is
	 * interested in processing a vncImage event implements this interface, and
	 * the object created with that class is registered with a component using
	 * the component's <code>addVncImageListener<code> method. When
	...
	void decodeGradientData(int x, int y, int w, int h, byte[] buf) {
		...
		for (dy = 0; dy < h; dy++) {
			...
			for (dx = 1; dx < w; dx++) {
				for (c = 0; c < 3; c++) {
					est[c] = (((prevRow[(dx * 3) + c] & 0xFF) + (pix[c] & 0xFF)) - (prevRow[((dx - 1) * 3)
							+ c] & 0xFF));
					if (est[c] > 0xFF) {
						est[c] = 0xFF;
					} else if (est[c] < 0x00) {
	...
	void handleTightRect(int x, int y, int w, int h) throws Exception {
		...
		if (comp_ctl == RfbProto.TightJpeg) {
			...
			// Read JPEG data.
			jpegData = new byte[rfb.readCompactLen()];
		...
		if (dataSize < RfbProto.TightMinToCompress) {
			...
			if (numColors != 0) {
				...
				if (bytesPixel == 1) {
					for (int dy = y; dy < (y + h); dy++) {
						rfb.readFully(pixels8, (dy * rfb.framebufferWidth) + x,
								w);
						if (rfb.rec != null) {
	...
	void handleZlibRect(int x, int y, int w, int h) throws Exception {
		...
		if (bytesPixel == 1) {
			for (int dy = y; dy < (y + h); dy++) {
				zlibInflater.inflate(pixels8, (dy * rfb.framebufferWidth) + x,
						w);
				if ((rfb.rec != null) && !rfb.recordFromBeginning) {
					rfb.rec.write(pixels8, (dy * rfb.framebufferWidth) + x, w);
				}
	...
	public void paint(Graphics g) {
		synchronized (memImage) {
			...
			if (rfb.framebufferWidth == scaledWidth) {
				...
				for (VncImageListener listener : listeners) {
					listener.imageCreated(memImage);
				}
			} else {
	...
	public void processLocalMouseEvent(MouseEvent evt, boolean moved) {
		if ((viewer.rfb != null) && rfb.inNormalProtocol) {
			...
			if (rfb.framebufferWidth != scaledWidth) {
				int sx = ((evt.getX() * 100) + (scalingFactor / 2))
						/ scalingFactor;
				int sy = ((evt.getY() * 100) + (scalingFactor / 2))
						/ scalingFactor;
				...
				case RfbProto.FramebufferUpdate:
					...
					for (int i = 0; i < rfb.updateNRects; i++) {
						...
						if ((rfb.updateRectEncoding == RfbProto.EncodingXCursor)
							...
							handleCursorShapeUpdate(rfb.updateRectEncoding, rx,
									ry, rw, rh);
							continue;
						}
					...
					// Defer framebuffer update request if necessary. But wake
					// up
					...
					rfb.writeFramebufferUpdateRequest(0, 0, w, h,
							!fullUpdateNeeded);
