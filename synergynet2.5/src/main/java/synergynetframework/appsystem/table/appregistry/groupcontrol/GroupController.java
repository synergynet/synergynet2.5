...
public class GroupController {
	...
	public GroupController(SynergyNetApp app) {
		...
		backgroundController = (SketchPad) contentSystem
				.createContentItem(SketchPad.class);
		backgroundController.setWidth(DisplaySystem.getDisplaySystem()
				.getWidth());
		backgroundController.setHeight(DisplaySystem.getDisplaySystem()
				.getHeight());
		backgroundController.setSketchArea(new Rectangle(0, 0,
				backgroundController.getWidth(), backgroundController
						.getHeight()));
			...
			public void cursorReleased(ContentItem item, long id, float x,
				...
				for (DrawData data : backgroundController.getDrawData()) {
					if (data instanceof DrawLine) {
						...
						if (groupBounds.containsKey(id)
								&& (line.getCursorId() == id)) {
							gb = groupBounds.get(id);
							if (line.getStartPoint().getX() < gb.minX) {
								gb.minX = line.getStartPoint().x;
							}
							if (line.getStartPoint().getX() > gb.maxX) {
								gb.maxX = line.getStartPoint().x;
							}
							if (line.getStartPoint().getY() < gb.minY) {
								gb.minY = line.getStartPoint().y;
							}
							if (line.getStartPoint().getY() > gb.maxY) {
								gb.maxY = line.getStartPoint().y;
							}

							if (line.getEndPoint().getX() < gb.minX) {
								gb.minX = line.getEndPoint().x;
							}
							if (line.getEndPoint().getX() > gb.maxX) {
								gb.maxX = line.getEndPoint().x;
							}
							if (line.getEndPoint().getY() < gb.minY) {
								gb.minY = line.getEndPoint().y;
							}
							if (line.getEndPoint().getY() > gb.maxY) {
								gb.maxY = line.getEndPoint().y;
							}
						}
					}
				}
				...
				for (ContentItem contentItem : contentSystem
						.getAllContentItems().values()) {
					if (contentItem instanceof OrthoContentItem) {
						OrthoContentItem orthoItem = (OrthoContentItem) contentItem;
						if (!(orthoItem instanceof ObjShape)
								&& !orthoItem.getName().equals(
										backgroundController.getName())
								&& (orthoItem.getParent() == null)) {
							if ((orthoItem.getLocation().x >= gb.minX)
									&& (orthoItem.getLocation().x <= gb.maxX)
									&& (orthoItem.getLocation().getY() >= gb.minY)
									&& (orthoItem.getLocation().getY() <= gb.maxY)) {
				...
				if (!toGroup.isEmpty()) {
					...
					final Window group = (Window) contentSystem
							.createContentItem(Window.class);
					group.setLocation(gb.minX + ((gb.maxX - gb.minX) / 2),
							gb.minY + ((gb.maxY - gb.minY) / 2));
					group.setWidth(gb.maxX - gb.minX);
					group.setHeight(gb.maxY - gb.minY);
					for (ContentItem subItem : toGroup) {
						Vector3f wl = ((Spatial) subItem
								.getImplementationObject())
								.getWorldTranslation();
						Vector3f ll = new Vector3f();
						((Spatial) group.getImplementationObject())
								.worldToLocal(wl, ll);
						group.addSubItem(subItem);
						((Spatial) subItem.getImplementationObject())
								.setLocalTranslation(ll);
						((Spatial) group.getImplementationObject())
					...
					final SimpleButton closeButton = (SimpleButton) contentSystem
					...
					closeButton.drawImage(MathPadResources.class
							.getResource("buttons/close.png"), 0, 0, 18, 18);
					closeButton.setLocation(group.getBackgroundFrame()
							.getWidth() / 2, group.getBackgroundFrame()
							.getHeight() / 2);
					closeButton.addButtonListener(new SimpleButtonAdapter() {

						@Override
						public void buttonReleased(SimpleButton b, long id,
								float x, float y, float pressure) {
							for (ContentItem subItem : toGroup) {
								if (!subItem.getName().equals(
										closeButton.getName())
										&& !(subItem instanceof BackgroundController)) {
									group.detachSubItem(subItem);
								}
							}
							if (group.getParent() != null) {
								group.getParent().removeSubItem(group);
							} else {
								contentSystem.removeContentItem(group);
							}
						}
