...
public class EnumSerializer extends Serializer {
	 ...
	 *             the serialization exception
	 */
	static public void put(ByteBuffer buffer, Enum<?> value)
			throws SerializationException {
	...
	public <T> T readObjectData(ConnectionHandler connectionHandler,
		...
		if (enumConstants == null) {
			throw new SerializationException("Class is not an enum: "
					+ type.getName());
		}
		int ordinal = IntSerializer.get(buffer, true);
		if ((ordinal < 0) || (ordinal > (enumConstants.length - 1))) {
			throw new SerializationException("Invalid ordinal for enum \""
					+ type.getName() + "\": " + ordinal);
		}
