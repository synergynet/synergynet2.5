 ...
 */
public class JMFVideoImage extends Image implements ByteBufferRendererListener,
		ControllerListener {
	
	/** The Constant log. */
	private static final Logger log = Logger.getLogger(JMFVideoImage.class
			.getName());
	...
	/** The inittexture. */
	private transient boolean failed = false, ready = false,
			inittexture = false;
	...
	public JMFVideoImage(URL url, boolean loop, int scalemethod)
		...
		try {
			...
			fpc = (FramePositioningControl) jmfplayer
	...
	public void setSize(int videowidth, int videoheight, RGBFormat format) {
		...
		try {
			...
			data.clear();
			data.add(ByteBuffer.allocateDirect(size * size * 4).order(
	...
	public boolean update(Texture texture, boolean syncToFrameRate) {
		...
		if (inittexture && (this.scalemethod > 0)) {
			...
			if (scalemethod == SCALE_MAXIMIZE) {
				...
				texture.setScale(new Vector3f(videowidth * (1f / this.width),
						videoheight * (1f / this.height), 1f));
			}
		...
		synchronized (this) {
			...
			GL11.glTexSubImage2D(GL11.GL_TEXTURE_2D, 0, 0, 0, videowidth,
					videoheight, pixelformat, dataformat, buffer);
